...
function Header()
{
	...
	$this->Image('../../imagenes/logouniservicios.png' , 10 ,10, 40 , 20,'PNG');
	$this->SetFont('Arial','B',20);
	$this->Cell(80);
	$this->Cell(50,20,'Reporte de Afiliados',0,0,'C');
	$this->Ln(15);
...
$pdf->SetFont('Arial', '', 8);
//CONSULTA
$estudiantes = $dbconfig->query("SELECT * FROM afiliados");
$item = 0;
while($estudiantes2 = mysqli_fetch_array($estudiantes)){
	$item = $item+1;
	$pdf->Cell(10, 8, $item, 0);
	$pdf->Cell(20, 8, $estudiantes2['nombresA'], 0);
	$pdf->Cell(30, 8, utf8_decode($estudiantes2['apellidosA']), 0);
	$pdf->Cell(30, 8, utf8_decode($estudiantes2['cedulaA']), 0);
	$pdf->Cell(30, 8, $estudiantes2['telefonoA'], 0);
    $pdf->Cell(50, 8, $estudiantes2['correoA'], 0);
   	$pdf->Cell(20, 8, $estudiantes2['estadoA'], 0);
	$pdf->Ln(5);
}
$pdf->Ln(8);
